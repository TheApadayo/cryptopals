...
public class Set1
{
	...
	public static void challenge6() throws Exception
	{
		...
		// my guess is 5
		System.out.println("Best key sizes are: " + nrmlEditDistance.get(0).getKey() + "(" + nrmlEditDistance.get(0).getValue() + "), " + nrmlEditDistance.get(1).getKey() + "(" + nrmlEditDistance.get(1).getValue() + "), " + nrmlEditDistance.get(2).getKey() + "(" + nrmlEditDistance.get(2).getValue() + "), " + nrmlEditDistance.get(3).getKey() + "(" + nrmlEditDistance.get(3).getValue() + ")");
		...
		for (int i = 0; i < 1; i++)
		{
			...
			for (int k = 0; k < keysize; k++)
			{
				...
				for (int j = 0; j < 255; j++)
				{
					...
					double score = HexUtils.stringMetric(decoded);
					if (score > 0.85) // mess with this score to get values to
