...
public class AESCipher {
	...
	public void initData(byte[] data)
	{
		...
		if(data.length % 16 != 0)
		{
			byte[] blockdata = new byte[16];
			int j=0;
			while(pos < data.length) blockdata[j++] = data[pos++];
			// PKCS#7 padding
			if(paddingMode == PADDING_PKCS7)
			{
				byte pad = (byte)(16 - j);
				while(j < 16) blockdata[j++] = pad;
			}
	...
	{
		if(data.length != 16) throw new EncryptionException("IV must be same as block size! (128 bits)");
